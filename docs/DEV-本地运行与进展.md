...
- **方法级 RBAC**：`RoleGuard.requireAny(APPROVER)` 应用于规则审批/退回/推广等敏感操作；集成测试验证 GUEST 角色审批被拒（403）。
- **绩效单查询页** `PayslipView`：按周期查询已落库绩效单（幂等去重），展开查看逐项下钻明细；导航新增入口。
- **测试**：累计 91 个全绿（引擎30 + 指标5 + 集成9 + 核算8 + 治理8 + API31）。

## 已完成（绩效明细表 · 按日期不跨月）

- **后端**：`CommissionDetailEntity`/仓储/`CommissionController`——逐单提成明细，按 `period(yyyy-MM)` + 日区间查询；录入校验业务日期属于该周期、查询区间限当月，**天然不跨月**；租户隔离。测试 2 项（区间过滤命中/跨月录入被拒）。
- **前端** `CommissionView`：周期 + 日区间筛选，明细列表 + 合计；导航新增。
- **测试**：累计 93 个全绿（引擎30 + 指标5 + 集成9 + 核算8 + 治理8 + API33）。前端 10 页面构建通过。

## 已完成（员工端消息中心）

- **后端**：`NoticeEntity`/`NoticeService`/`NoticeController`——规则**生效即推送通知**（三级串签通过或到期生效），租户广播；集成测试验证生效后通知非空（复用三级串签流）。
- **前端** `NoticeView`：消息中心列表（含变更单号追溯），导航新增；前端 11 页面构建通过。
- **测试**：93 个全绿（引擎30 + 指标5 + 集成9 + 核算8 + 治理8 + API33）。

## 已完成（接口文档 + 后端 README）

- **OpenAPI/Swagger**：springdoc 集成，`/swagger-ui.html` + `/v3/api-docs`；集成测试验证文档端点可用（title 校验）。
- **后端 README** `backend/README.md`：模块职责、运行、认证与多租户、主要接口、安全达标、测试说明。
- **测试**：94 个全绿（引擎30 + 指标5 + 集成9 + 核算8 + 治理8 + API34）。
